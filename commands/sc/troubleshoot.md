...
- **Agent**: Delegate deep investigation to specialist agents (see Agent Delegation below)
...
- Multi-component bug with no obvious root cause after initial analysis, OR
- `--trace` on a crash/corruption spanning RC↔AC boundary, OR
- Issue description mentions auth, credentials, encryption, or injection (add `security-engineer`)

| Role | Lens |
|------|------|
| Analyst | Evidence-based hypothesis testing, eliminate assumptions, trace from symptom to cause |
| Security | Threat modeling, input validation, auth/authz, least privilege, data protection |

When spawning: use `general-purpose` subagent, include the role lens in the agent prompt. Single-component or obvious bugs — always inline. Reserve the Analyst lens for genuinely ambiguous multi-component failures where independent codebase search adds value.
